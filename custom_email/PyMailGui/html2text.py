...
class Parser(HTMLParser):              # subclass parser, define callback methods
    def __init__(self, convert_charrefs=True):                # text assumed to be str, any encoding ok
...
def html2text(text):
    try:
        hp = Parser(convert_charrefs=False) # MY VERSION(python3.8 require convert_charrefs to be false to override handle_entityref)
        hp.feed(text)
        return hp.text
    except:
        return text
...
if __name__ == '__main__': 
    ...
    import sys, tkinter, html
    try:
        text = open(sys.argv[1], 'r').read()
    except:
        text = "<p>html sample<a href='www.python.com'>python</a></p>"
    # text = html.escape("1<2 <b>hello & bye</b>")
    text = html2text(text)
    t = tkinter.Text()
    t.insert('1.0', text)
    t.pack()
