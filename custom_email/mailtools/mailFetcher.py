...
import poplib, sys  # client's mailconfig on sys.path
...
class MailFetcher(MailTool):
    ...
    def connect(self):
        ...
        server = poplib.POP3_SSL(self.popServer, timeout=20)
        ...
        if not self.srvrHasTop:  # not all servers support TOP
            ...
            try:
                ...
                for msgnum in range(loadfrom, msgCount + 1):  # poss empty
                    ...
                    print(progress, "---downloadAllHeaders")
            ...
            assert len(allhdrs) == len(allsizes)  # if False means not all massages checked
    ...
    def getPassword(self):
        ...
        if not self.popPassword:
            try:
                self.popPassword = mailconfig.pop_gmail_pass
            except:
