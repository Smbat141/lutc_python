...
from custom_email import mailconfig  # client's mailconfig
...
class MailSender(MailTool):
    ...
    def sendMessage(self, From, To, Subj, extrahdrs, bodytext, attaches,
        ...
        server = smtplib.SMTP_SSL(self.smtpServerName, timeout=20)  # this may fail too
    ...
    def authenticateServer(self, server):
        server.login(mailconfig.smtpuser, mailconfig.smtp_gmail_pass)
...
class MailSenderAuth(MailSender):
    ...
    def getPassword(self):
        ...
        if not self.smtpPassword:
            try:
                MailSenderAuth.smtpPassword = mailconfig.smtp_gmail_pass  # MY VERSION
...
class MailSenderAuthConsole(MailSenderAuth):
    def askSmtpPassword(self):
        ...
        if not prompt:
            prompt = mailconfig.smtp_gmail_pass
