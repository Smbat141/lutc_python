...
def sendmessage():
    ...
    server.login("smbatpaloyan@gmail.com", mailconfig.smtp_gmail_pass)
    try:
...
def loadmessages(servername, user, passwd, loadfrom=1):
    ...
    try:
        ...
        # for i in range(loadfrom, msgCount+1):  # empty if low >= high | MY VERSION
        for i in range(loadfrom, 2):  # MY VERSION
