...
audiotools = {
    ...
    'linux': Cmdline('cat %s > /dev/audio'),  # on zaurus, at least
...
videotools = {
    'linux': Cmdline('xdg-open %s'),  # zaurus pda
    'win32': Winstart()  # avoid DOS pop up
}

imagetools = {
    'linux': Cmdline('eog %s'),  # zaurus pda
    'win32': Winstart()
}

texttools = {
    'linux': Cmdline('nano %s'),  # zaurus pda
...
if __name__ == '__main__':
    ...
    playfile('bit_byte.png')
    playfile('OReilly.Programming.Python.4.Edition.Dec.2010.pdf')
    playfile('mysql.mp4')
