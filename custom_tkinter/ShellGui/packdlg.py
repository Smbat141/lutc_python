...
from custom_tkinter.ShellGui.packer import pack                         # use pack script/module
from custom_tkinter.ShellGui.formrows import makeFormRow                # use form builder tool
...
def packDialog():                               # a new top-level window
    ...
    win.wait_visibility()                      # ADDED THIS COMMAND MYSELF(there was error in Lutz version)
    win.grab_set()
...
def runPackDialog():
    ...
    if output != "" and patterns != "":              # till ok or wm-destroy
        ...
        print('Packer:', output, filenames)          # OPEN NEW WINDOW WHEN PRINTED
