...
def formatlink(cgiurl, parmdict):
    ...
    note that URL escapes are different from HTML (escape)
    """
...
def messagearea(headers, text, extra=''):  # extra for readonly
    ...
    print('%s\n</textarea></table>' % (escape(text) or '?'))  # if has </>s
...
def viewpage(msgnum, headers, text, form, parts=[]):
    ...
    in HTML here, so we use escape; this usually sends nonprintable
    ...
    user, pswd, site = list(map(escape, getstandardpopfields(form)))
...
def errorpage(message, stacktrace=True):
    ...
    print('<h2>Python Exception</h2><p>', escape(str(exc_type)))
    print('<h2>Exception details</h2><p>', escape(str(exc_value)))
    if stacktrace:
...
def getstandardpopfields(form):
    ...
    return (getfield(form, 'user', mailconfig.popusername),
            getfield(form, 'pswd', mailconfig.pop_gmail_pass),
